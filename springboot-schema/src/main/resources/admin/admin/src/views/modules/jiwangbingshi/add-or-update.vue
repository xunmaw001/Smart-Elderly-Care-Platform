        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.guominshi" label="过敏史" prop="guominshi">
                    <span>{{ruleForm.guominshi}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="遗传史" prop="yichuanshi">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="遗传史"
                  v-model="ruleForm.yichuanshi" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yichuanshi" label="遗传史" prop="yichuanshi">
                    <span>{{ruleForm.yichuanshi}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.guominshi = true;
          ...
          if(o=='yichuanshi'){
            this.ruleForm.yichuanshi = obj[o];
	    this.ro.yichuanshi = true;
            continue;
          }
          if(o=='dengjishijian'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiwangbingshi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiwangbingshiCrossAddOrUpdateFlag = false;
						 this.parent.search();
